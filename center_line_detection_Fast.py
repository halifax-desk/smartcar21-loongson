...
def Load_Image(filename):
    ...
    gray = cv2.cvtColor(global_img, cv2.COLOR_BGR2GRAY)     // 转换为灰度图
    
    _, global_binary = cv2.threshold(gray, BINARY_THRESHOLD, 255, cv2.THRESH_BINARY)        // 二值化
    
    binary_output_path = os.path.join(OUTPUT_DIR, f"binary_{filename}")
    cv2.imwrite(binary_output_path, global_binary)
    
    column_white_counts = np.sum(global_binary > 0, axis=0)     // 计算每列白色像素的数量
    global_reference_col = np.argmax(column_white_counts)     // 创建参考列
    
    global_left_boundary_count = 0       
    global_right_boundary_count = 0     // 初始化右边界计数
...
def process_images():
    ...
    for filename in bmp_files:
        ...
        filtered_edges = np.zeros_like(global_binary)       // 创建与二值化图像相同大小的零矩阵
